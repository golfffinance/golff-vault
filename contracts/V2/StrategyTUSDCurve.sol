...
contract StrategyTUSDCurve {
    ...
    address public gycrv ;// ycrv vault
    ...
    constructor(address _controller, address _gycrv) public {
    ...
    function getName() external pure returns (string memory) {
        return "golff:StrategyTUSD:Curve";
    }
    ...
    function deposit() public {
        ...
        if (_ycrv > 0) {
            IERC20(ycrv).safeApprove(gycrv, 0);
            IERC20(ycrv).safeApprove(gycrv, _ycrv);
            yERC20(gycrv).deposit(_ycrv);
        }
    }
    ...
    function _withdrawAll() internal {
        uint _gycrv = IERC20(gycrv).balanceOf(address(this));
        if (_gycrv > 0) {
            yERC20(gycrv).withdraw(_gycrv);
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint) {
        ...
        // calculate amount of gycrv to withdraw for amount of _ycrv_
        uint _gycrv = _ycrv.mul(1e18).div(yERC20(gycrv).getPricePerFullShare());
    ...
    function balanceOfGYCRV() public view returns (uint) {
        return IERC20(gycrv).balanceOf(address(this));
    }
    
    function balanceOfGYCRVinYCRV() public view returns (uint) {
        return balanceOfGYCRV().mul(yERC20(gycrv).getPricePerFullShare()).div(1e18);
    }
    
    function balanceOfGYCRVinyTUSD() public view returns (uint) {
        return balanceOfGYCRVinYCRV().mul(ICurveFi(curve).get_virtual_price()).div(1e18);
    }
