...
contract GOFStrategyPool {
    ...
    function harvest() public {
        ...
        YFIIPool(pool).getReward(); 
        address _vault = IGOFStrategyController(controller).vaults(address(want));
        require(_vault != address(0), "Golff:!vault"); // additional protection so we don't burn the funds
        
        //output -> eth -> gof
        uint _gr = IERC20(output).balanceOf(address(this));
        if (_gr > 0) {
            IERC20(output).safeApprove(uni, 0);
            IERC20(output).safeApprove(uni, _gr);
            Uniswap(uni).swapExactTokensForTokens(_gr, 0, swapRouting, address(this), now.add(1800));
        }
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint) {
        YFIIPool(pool).withdraw(_amount);
        return _amount;
    }
    
    function balanceOf() public view returns (uint) {
        return YFIIPool(pool).balanceOf(address(this));
    }

    function balanceOfPendingReward() public view returns(uint){ 
        return YFIIPool(pool).earned(address(this));
    }
