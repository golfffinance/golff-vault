...
/**
 * @dev GOF DForece GOFStrategy
 * Dai -> dDai -> Df -> ETH -> Gof
 * Usdt -> dUsdt -> Df -> Eth -> Gof
 */
contract GOFStrategyDForceDai {
    ...
    address constant public df = address(0x431ad2ff6a9C365805eBaD47Ee021148d6f7DBe0);//dforce mint coins
    address constant public uni = address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);// uniswap
    ...
    constructor(address _controller, address _want, address _pool, address _dtoken) public {
    ...
    function deposit() external {
        ...
        uint _d = IERC20(dtoken).balanceOf(address(this));
        if (_d > 0) {
    ...
    function harvest() public {
        ...
        if (_df > 0) {
            ...
            Uniswap(uni).swapExactTokensForTokens(_df, uint(0), path, address(this), now.add(1800));
        }
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint) {
        uint _d = _amount.mul(1e18).div(dERC20(dtoken).getExchangeRate());
