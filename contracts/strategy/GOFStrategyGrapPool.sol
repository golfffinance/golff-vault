...
contract GOFStrategyGrapPool {
    ...
    function init() public{
        IERC20(output).safeApprove(uni, uint(-1));
    }
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint) {
        GrapPool(pool).withdraw(_amount);
        return _amount;
    }
    
    function balanceOf() public view returns (uint) {
        return GrapPool(pool).balanceOf(address(this));
    }

    function balanceOfPendingReward() public view returns(uint){ 
        return GrapPool(pool).earned(address(this));
    }
